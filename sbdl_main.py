...
if __name__ == '__main__':
    ...
    contract_df = Transformation.get_contract(accounts_df)
    ...
    relation_address_df = Transformation.get_address(address_df)

    logger.info("Join Party Relations and Address")
    party_address_df = Transformation.join_party_address(relations_df, relation_address_df)

    logger.info("Join Account and Parties")
    data_df = Transformation.join_contract_party(contract_df, party_address_df)
    logger.info("Apply Header and create Event")
    final_df = Transformation.apply_header(spark, data_df)
